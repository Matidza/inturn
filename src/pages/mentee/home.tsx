...
import {
  ...
  LinearProgress, Chip, Avatar, Skeleton, Tooltip, Alert,
} from "@mui/material";
...
async function apiFetch<T>(path: string): Promise<T> {
  // Was reading "authToken" — the auth provider actually stores the
  // token under the key "token" on login. That mismatch meant the
  // Authorization header was never sent.
  const token = localStorage.getItem("token") ?? "";
  const res = await fetch(`${API_BASE}${path}`, {
    // Required for the gateway's httpOnly accessToken cookie to be sent
    // at all — frontend (5173) and gateway (1000) are different origins,
    // so fetch() won't attach cookies cross-origin without this.
    credentials: "include",
    headers: {
        ...
        <Stack direction={{ xs: "column", md: "row" }} justifyContent="space-between"
          ...
          <Stack direction="row" spacing={1.3} flexWrap="wrap">
            <Button component={Link} to="/mentee/ai-home" variant="contained" sx={{
              ...
              fontWeight: 600, px: 2, fontSize: 14, fontFamily: "'DM Sans',sans-serif",
            ...
            <Button component={Link} to="/mentee/professionals" variant="outlined" sx={{
              ...
              borderColor: t.btnOutline, color: PURPLE, fontWeight: 600, px: 2, fontSize: 14,
