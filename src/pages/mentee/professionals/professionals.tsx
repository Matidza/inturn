  ...
  return (
    ...
    >
      ...
      >
        {pro.image ? (
          ...
          <Box
            sx={{
              ...
              alignItems: "center",
              justifyContent: "center",
              background: `linear-gradient(135deg, ${PRIMARY_LIGHT}, #e0d4fb)`,
            }}
          >
            <Avatar
              sx={{
        ...
        >
          {liked
            ? <FavoriteFilled sx={{ fontSize: 16, color: "#ef4444" }} />
            : <FavoriteIcon sx={{ fontSize: 16, color: "#555" }} />
          }
        </IconButton>
      ...
      <Box
        onClick={onClick}
        sx={{ p: 1.5, display: "flex", flexDirection: "column", flex: 1, minWidth: 0, overflow: "hidden" }}
      >
        {/* Seller info row */}
        <Stack direction="row" spacing={1} alignItems="center" mb={1} sx={{ minWidth: 0 }}>
          <Avatar
            src={pro.image}
            sx={{ width: 28, height: 28, bgcolor: PRIMARY, fontSize: 11 }}
          >
            {!pro.image && getInitials(pro.name, pro.surname)}
          </Avatar>

          <Typography fontSize={13} fontWeight={600} noWrap flex={1}>
            {pro.name} {pro.surname}
          </Typography>

          {sellerLevel && (
            <Chip
              label={sellerLevel.label}
              size="small"
              sx={{
                fontSize: 10,
                fontWeight: 700,
                height: 20,
                bgcolor: sellerLevel.bg,
        ...
        {/* Gig title — like Fiverr's "I will..." */}
        <Typography
          fontSize={13}
          color="#404145"
          sx={{
        ...
        >
          {pro.aboutUser ||
            `I will conduct a professional mock interview for ${pro.interviewFocusArea?.[0] ?? "your target role"}`}
        </Typography>
        ...
        {/* Rating + price row — exactly like Fiverr */}
        <Stack direction="row" justifyContent="space-between" alignItems="center">
          ...
          <Box textAlign="right">
            ...
            <Typography fontSize={14} fontWeight={800} color="#404145">
  ...
  >
    ...
    <Stack spacing={3}>
      ...
      <Box>
        ...
        <Slider
          value={priceRange}
          onChange={(_: any, val: any) => setPriceRange(val)}
          valueLabelDisplay="auto"
          valueLabelFormat={(v: number) => `R${v}`}
          min={0}
          max={dataMaxPrice}
          sx={{ color: PRIMARY }}
        />
          ...
          {[0, 4, 4.5, 4.8].map((r) => (
            <Box
              ...
              sx={{
                ...
                borderRadius: 2,
                cursor: "pointer",
                border: minRating === r ? `2px solid ${PRIMARY}` : "2px solid transparent",
                bgcolor: minRating === r ? PRIMARY_LIGHT : "transparent",
            ...
            >
              ...
              />
              <Typography fontSize={13} fontWeight={minRating === r ? 700 : 400}>
                {r === 0 ? "Any rating" : `${r}+`}
              </Typography>
            </Box>
          ))}
...
const Professionals: React.FC = () => {
  ...
  const hasActiveFilters =
    !!filterText || selectedCategory !== "All" || minRating > 0 ||
  ...
  return (
    <Box sx={{ backgroundColor: "#fff", minHeight: "100vh", overflowX: "hidden", maxWidth: "100vw", width: "100%" }}>
      ...
      >
        <Stack direction={{ xs: "column", sm: "row" }} spacing={1.5} alignItems="center">
          ...
          <Button
            ...
            sx={{
              textTransform: "none",
              borderColor: hasActiveFilters ? PRIMARY : "#ddd",
              color: hasActiveFilters ? PRIMARY : "#555",
              borderRadius: 3,
          ...
          {hasActiveFilters && (
            <Button
              ...
              sx={{ textTransform: "none", color: PRIMARY, whiteSpace: "nowrap", flexShrink: 0 }}
            >
              Clear
            </Button>
          )}
      ...
      {/* ── Category chips (Fiverr top nav style) ── */}
      <Box
        sx={{
          px: { xs: 2, md: 4 },
          py: 1.5,
      ...
      >
        {loading
          ? Array.from({ length: 6 }).map((_, i) => (
              <Skeleton key={i} variant="rounded" width={90} height={32} sx={{ borderRadius: 10, flexShrink: 0 }} />
            ))
          : categories.map((cat) => (
              <Chip
                key={cat}
                ...
                sx={{
                  ...
                  backgroundColor: selectedCategory === cat ? P_LITE : "transparent",
                  color: selectedCategory === cat ? P : "#555",
                  border: selectedCategory === cat ? `1px solid ${P}` : `1px solid ${BORDER}`,
                  "&:hover": {
                    backgroundColor: selectedCategory === cat ? P_LITE : "#f5f5f5",
                  },
      ...
      >
        ...
        {/* Desktop sort */}
        <FormControl size="small" sx={{ display: { xs: "none", md: "flex" }, minWidth: 170 }}>
          <InputLabel>Sort By</InputLabel>
          <Select
            value={sortBy}
            label="Sort By"
      ...
      {/* ── Grid ── */}
      <Box px={{ xs: 2, md: 4 }} pb={6} sx={{ overflowX: "hidden" }}>
        {loading ? (
      ...
      {/* ── Filter drawer (slides from right like Fiverr) ── */}
      <FilterDrawer
